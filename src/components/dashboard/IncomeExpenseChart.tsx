...
interface IncomeExpenseChartProps {
    /** Monthly trend data for income and expenses. */
    data: MonthlyTrend[];
}

/**
 * Custom tooltip for the income vs expenses chart.
 */
...
const TrendTooltip: React.FC<TrendTooltipProps> = ({ active, payload, label }) => {
    if (active && payload && payload.length > 0) {
        return (
            ...
            >
                <p style={{ color: 'var(--cds-text-primary)', fontWeight: 600, marginBottom: '0.5rem', borderBottom: '1px solid rgba(255,255,255,0.1)', paddingBottom: '0.25rem' }}>
                    {label}
                </p>
                {payload.map((entry) => (
                    <div key={entry.name} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '1rem', marginTop: '0.25rem' }}>
                        <span style={{ color: 'var(--cds-text-secondary)', fontSize: '0.8125rem', display: 'flex', alignItems: 'center', gap: '0.375rem' }}>
                            <div style={{ width: 8, height: 8, borderRadius: '50%', background: entry.fill }} />
                            {entry.name}
                        </span>
                        <span style={{ color: entry.fill, fontWeight: 600, fontSize: '0.875rem' }}>
...
/**
 * IncomeExpenseChart renders a bar chart comparing income vs expenses monthly.
 */
        ...
        <div className="kunji-card" role="img" aria-label="Income versus expenses trend chart">
            ...
            <ResponsiveContainer width="100%" height={280}>
                <BarChart data={data} barGap={6}>
                    ...
                    <XAxis
                        dataKey="month"
                        tick={{ fill: 'var(--cds-text-secondary)', fontSize: 13, fontWeight: 500 }}
